...
function preload() {
  img = loadImage('../assets/clouds.png');
  img2 = loadImage('../assets/clouds.png');
}
...
function draw() {
  ...
  let num = 8; // number of crops
  let startingAngle = 180; // angle of each crop
  ...
  for (let i = 0; i < num; i++) {
    ...
    mask.noStroke();
    mask.fill(255);
    // Positions the mask in the middle and sets the size of the mask as a diminishing size based on the number of steps in the loop
